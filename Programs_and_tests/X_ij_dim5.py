...
class HybridFunction(Function):
     ...
     def forward(ctx, input, quantum_circuit, shift, dimension):
        # ctx è un oggetto che viene creato ed inserito automaticamente da PyTorch quando eseguiamo il forward pass.
        ctx.shift = shift
        ...
        # è perchè la loss function richiede una fromattazione specifica
        return result
     ...
     def backward(ctx, grad_outputs): # grad_outputs è il gradiente proveniente dai layer classici successivi
        ...
        grad_input = torch.sum(grad_input_matrix, -1) 
        
        # Ritorno il gradiente con lo stesso shape dell'input iniziale, e 'None' per gli altri argomenti non addestrabili di forward() (quantum_circuit, shift e dimension)
...
class Net(nn.Module):
     def __init__(self, dimension, shots, shift):
          ...
          self.layer_3 = nn.Linear(64, dimension) #l'ultima riduzione fa si che i parametri di output del secondo layer diventino dello stesso numero della dimensione del qudit
     ...
     def forward(self, x):
          ...
          # Non applichiamo la ReLU qui perché gli angoli di rotazione quantistici devono poter assumere anche valori negativi (da -pi a pi).
          x = self.layer_3(x) 
...
def training_loop(n_epochs, optim, model, loss_func, train_loader):
     ...
     for epoch in range(n_epochs):
          ...
          start_time = time.perf_counter()  # Registra il tempo di inizio epoca
...
for i in range(number_of_test):
    ...
    #raccolgo 1000 immagini per ogni classe (0, 1, 2, 3, 4)
    n_val_samples = 1000
